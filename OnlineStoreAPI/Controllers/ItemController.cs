...
namespace OnlineStoreAPI.Controllers
{
    ...
    public class ItemController : ControllerBase
    {
        ...
        [HttpGet]
        public async Task<ActionResult<ResponseDTO<IEnumerable<ItemShortDTO>>>> GetItems(int pageNumber = 1, int pageSize = 50)
        {
            ResponseDTO<IEnumerable<ItemShortDTO>> result = new ResponseDTO<IEnumerable<ItemShortDTO>>(null);
            try
            {
                result = await _itemServices.GetAsync(pageNumber, pageSize);
                return Ok(result);
            }
            catch (Exception ex)
            {
        ...
        [HttpPut]
        public async Task<ActionResult<ResponseDTO<ItemDTO>>> UpdateItem(ItemUpdateDTO data)
        {
            ResponseDTO<ItemDTO> result = new ResponseDTO<ItemDTO>(null);
            try
            {
